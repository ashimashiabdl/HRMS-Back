...
{
    public class EducationFieldService : BaseService<EducationField, BaseInfoContext, EducationFieldDTO>, IScopedServices
    {
        private const int LazySearchMinFilterLength = 2;
        private const int LazySearchMaxResults = 30;

        public EducationFieldService(IMapper mapper, IUnitOfWork<BaseInfoContext> unitOfWork, IDapper dapper, IConfiguration configuration, UserResolverService userService)
            : base(unitOfWork.Context, mapper, unitOfWork, dapper, configuration, userService)
        {
        }

        /// <summary>
        /// جستجوی تنبل رشته تحصیلی — بدون فیلتر خالی برنمی‌گرداند تا بار اولیه سنگین نشود.
        /// </summary>
