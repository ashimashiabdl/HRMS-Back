...
[Route("api/FieldDataType")]
[ControllerGroup("Report", "گزارش گیری")]
[DisplayName("نوع داده فیلد")]
public class FieldDataTypeController : AppBaseController
{
    private readonly FieldDataTypeService _service;

    public FieldDataTypeController(FieldDataTypeService service, ILogger<FieldDataTypeController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService userResolverService)
    ...
    public async Task<IActionResult> Post([FromBody] FieldDataTypeDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] FieldDataTypeDTO body)
    {
        if (ModelState.IsValid)
        {
            try
