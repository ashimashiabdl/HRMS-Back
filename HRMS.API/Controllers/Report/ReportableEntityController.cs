...
[Route("api/ReportableEntity")]
[ControllerGroup("Report", "گزارش گیری")]
[DisplayName("موجودیت قابل گزارش")]
public class ReportableEntityController : AppBaseController
{
    private readonly ReportableEntityService _service;

    public ReportableEntityController(ReportableEntityService service, ILogger<ReportableEntityController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService userResolverService)
    ...
    public async Task<IActionResult> Post([FromBody] ReportableEntityDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] ReportableEntityDTO body)
    {
        if (ModelState.IsValid)
        {
            try
    ...
    {
        return this.AppOk(_service.GetAvailableTables());
    }
}
