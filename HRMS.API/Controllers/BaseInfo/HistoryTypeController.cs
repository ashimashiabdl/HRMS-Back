...
[Route("api/HistoryType")]
[ControllerGroup("baseInfo", "اطلاعات پایه ")]
[DisplayName("نوع سابقه")]
public class HistoryTypeController : AppBaseController
{
    private readonly HistoryTypeService _service;
    public HistoryTypeController(HistoryTypeService service, ILogger<HistoryTypeController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _service = service;
    ...
    public async Task<IActionResult> Post([FromBody] HistoryTypeDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] HistoryTypeDTO body)
    {
        if (ModelState.IsValid)
        {
            try
