...
[DisplayName("رده")]
public class RankController : AppBaseController
{
    private readonly RankService _service;
    public RankController(RankService service, ILogger<RankController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _service = service;
    ...
    public async Task<IActionResult> Post([FromBody] RankDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] RankDTO body)
    {
