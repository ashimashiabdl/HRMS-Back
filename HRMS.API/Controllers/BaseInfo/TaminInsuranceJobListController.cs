...
[Route("api/BaseTaminInsuranceJobListController")]
[ControllerGroup("baseInfo", "اطلاعات پایه ")]
[DisplayName("فهرست شغل های تامین اجتماعی")]
public class BaseTaminInsuranceJobListController : AppBaseController
{
    private readonly TaminInsuranceJobListService _TaminInsuranceJobListService;
    public BaseTaminInsuranceJobListController(TaminInsuranceJobListService TaminInsuranceJobListService, ILogger<BaseTaminInsuranceJobListController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
    ...
    {
        return Ok(_TaminInsuranceJobListService.GetAsKeyValuePair(""));
    }
