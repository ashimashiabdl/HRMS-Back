...
[Route("api/StaffingRule")]
[ControllerGroup("baseInfo", "اطلاعات پایه ")]
[DisplayName("قانون های استخدامی")]
public class StaffingRuleController : AppBaseController
{
    private readonly StaffingRuleService _baseTableService;
    public StaffingRuleController(StaffingRuleService StaffingRuleService, ILogger<StaffingRuleController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _baseTableService = StaffingRuleService;
    }
    ...
    public async Task<IActionResult> Post([FromBody] StaffingRuleDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] StaffingRuleDTO body)
    {
        if (ModelState.IsValid)
        {
            try
