...
public class JobGroupController(JobGroupService JobGroupService, ILogger<JobGroupController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : AppBaseController(UserResolverService, logger, accessor, mapper, dapper)
{
    private readonly JobGroupService _baseTableService = JobGroupService;
    ...
    public async Task<IActionResult> Post([FromBody] JobGroupDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] JobGroupDTO body)
    {
        if (ModelState.IsValid)
        {
            try
