...
[DisplayName("نوع صندوق")]
public class FundTypeController : AppBaseController
{
    private readonly FundTypeService _service;
    public FundTypeController(FundTypeService service, ILogger<FundTypeController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _service = service;
    ...
    public async Task<IActionResult> Post([FromBody] FundTypeDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] FundTypeDTO body)
    {
        if (ModelState.IsValid)
        {
            try
