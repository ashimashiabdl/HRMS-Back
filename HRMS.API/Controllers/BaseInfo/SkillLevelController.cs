...
public class SkillLevelController : AppBaseController
{
    private readonly SkillLevelService _service;
    public SkillLevelController(SkillLevelService service, ILogger<SkillLevelController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _service = service;
    ...
    public async Task<IActionResult> Post([FromBody] SkillLevelDTO body)
    {
    ...
    public async Task<IActionResult> Put([FromBody] SkillLevelDTO body)
    {
        if (ModelState.IsValid)
        {
            try
