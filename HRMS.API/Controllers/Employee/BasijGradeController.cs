...
[Route("api/BasijGrade")]
[ControllerGroup("Employee", "اطلاعات کارکنان ")]
[DisplayName("ایجاده بسیج")]
[EmployeeAccessCheck]
public class BasijGradeController : AppBaseController
{
    private readonly BasijGradeService _BasijGradeService;
    private IConfiguration _configuration;
    private readonly EmployeeService _EmployeeService;

    public BasijGradeController(IConfiguration configuration, BasijGradeService Service, EmployeeService EmployeeService, ILogger<EmployeeController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _BasijGradeService = Service;
        _configuration = configuration;
        _EmployeeService = EmployeeService;
        _BasijGradeService._currentUserDefaultOrganId = currentUserDefaultOrganId;
    }
    ...
    {
        return this.AppOk(_BasijGradeService.Get(id));
    }
    [HttpPost("Post")]
    [CustomAccessKey(AccessKey: "create")]

    public async Task<IActionResult> Post([FromBody] BasijGradeDTO body)
    {
        body.OrganisationChartId = currentUserDefaultOrganId;
        return Ok(await _BasijGradeService.CreateForAsync(body));
    }
    [HttpPut("Put")]
    [CustomAccessKey(AccessKey: "update")]

    public async Task<IActionResult> Put([FromBody] BasijGradeDTO body)
    {
        body.OrganisationChartId = currentUserDefaultOrganId;
        var result = await _BasijGradeService.UpdateForAsync(body);
    ...
    public IActionResult GetPagedData(int currentPage = 0, int pageSize = 10, [FromQuery] string filter = "", [FromQuery] string activeSortColumn = "", [FromQuery] string Sortdirection = "", [FromQuery] bool IgnoreExpired = true, [FromQuery] long? EmployeeId = null)
    {
        ...
        return this.AppOk(_BasijGradeService.GetPagedData(currentPage: currentPage, pageSize: pageSize, filter, activeSortColumn, Sortdirection, IgnoreExpired, EmployeeId: EmployeeId));
    ...
    {
        return this.AppOk(_BasijGradeService.DeleteRecord(id));
    }
}
