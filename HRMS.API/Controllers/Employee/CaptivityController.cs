...
[DisplayName("اسارت")]
[EmployeeAccessCheck]
public class CaptivityController : AppBaseController
{
    private readonly CaptivityService _CaptivityService;
    private IConfiguration _configuration;
    private readonly EmployeeService _EmployeeService;

    public CaptivityController(IConfiguration configuration, CaptivityService Service, EmployeeService EmployeeService, ILogger<EmployeeController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _CaptivityService = Service;
        _configuration = configuration;
        _EmployeeService = EmployeeService;
        _CaptivityService._currentUserDefaultOrganId = currentUserDefaultOrganId;
    }
    ...
    {
        return this.AppOk(_CaptivityService.Get(id));
    }
    [HttpPost("Post")]
    [CustomAccessKey(AccessKey: "create")]

    public async Task<IActionResult> Post([FromBody] CaptivityDTO body)
    {
        body.OrganisationChartId = currentUserDefaultOrganId;
        return Ok(await _CaptivityService.CreateForAsync(body));
    }
    [HttpPut("Put")]
    [CustomAccessKey(AccessKey: "update")]

    public async Task<IActionResult> Put([FromBody] CaptivityDTO body)
    {
        if (ModelState.IsValid)
        {
            ...
            {
                body.OrganisationChartId = currentUserDefaultOrganId;
                var result = await _CaptivityService.UpdateForAsync(body);
    ...
    public IActionResult GetPagedData(int currentPage = 0, int pageSize = 10, [FromQuery] string filter = "", [FromQuery] string activeSortColumn = "", [FromQuery] string Sortdirection = "", [FromQuery] bool IgnoreExpired = true, [FromQuery] long? EmployeeId = null)
    {
        if (ModelState.IsValid)
        {
            try
            {
                ...
                return this.AppOk(_CaptivityService.GetPagedData(currentPage: currentPage, pageSize: pageSize, filter, activeSortColumn, Sortdirection, IgnoreExpired, EmployeeId: EmployeeId));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, ex.Message);
    ...
    {
        return this.AppOk(_CaptivityService.DeleteRecord(id));
    }
}
