...
public class ContactInfoController : AppBaseController
{
    ...
    {
        return this.AppOk(_ContactInfoService.Get(id));
    }
    [HttpPost("Post")]
    [CustomAccessKey(AccessKey: "create")]

    public async Task<IActionResult> Post([FromBody] ContactInfoDTO body)
    {
        if (ModelState.IsValid)
        {
            try
            {
                ...
                return Ok(await _ContactInfoService.CreateForAsync(body));
            }
            catch (Exception ex)
            {
    ...
    public async Task<IActionResult> Put([FromBody] ContactInfoDTO body)
    {
        if (ModelState.IsValid)
        {
            try
            {
                body.OrganisationChartId = currentUserDefaultOrganId;
                var result = await _ContactInfoService.UpdateForAsync(body);
    ...
    public IActionResult GetPagedData(int currentPage = 0, int pageSize = 10, [FromQuery] string filter = "", [FromQuery] string activeSortColumn = "", [FromQuery] string Sortdirection = "", [FromQuery] bool IgnoreExpired = true, [FromQuery] long? EmployeeId = null)
    {
        if (ModelState.IsValid)
        {
            try
            {
                ...
                return this.AppOk(_ContactInfoService.GetPagedData(currentPage: currentPage, pageSize: pageSize, filter, activeSortColumn, Sortdirection, IgnoreExpired, EmployeeId: EmployeeId));
    ...
    {
        return this.AppOk(_ContactInfoService.DeleteRecord(id));
    }
}
