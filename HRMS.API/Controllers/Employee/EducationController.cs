...
[DisplayName("تحصیلات")]
[EmployeeAccessCheck]
public class EducationController : AppBaseController
{
    private readonly EducationService _EducationService;
    private IConfiguration _configuration;
    private readonly EmployeeService _EmployeeService;

    public EducationController(IConfiguration configuration, EducationService Service, EmployeeService EmployeeService, ILogger<EmployeeController> logger, IHttpContextAccessor accessor, IMapper mapper, IDapper dapper, UserResolverService UserResolverService) : base(UserResolverService, logger, accessor, mapper, dapper)
    {
        _EducationService = Service;
        _configuration = configuration;
        _EmployeeService = EmployeeService;
        _EducationService._currentUserDefaultOrganId = currentUserDefaultOrganId;
    }
    ...
    {
        return this.AppOk(_EducationService.Get(id));
    }
    [HttpPost("Post")]
    [CustomAccessKey(AccessKey: "create")]

    public async Task<IActionResult> Post([FromBody] EducationDTO body)
    {
        body.OrganisationChartId = currentUserDefaultOrganId;
        return Ok(await _EducationService.CreateForAsync(body));
    }
    [HttpPut("Put")]
    [CustomAccessKey(AccessKey: "update")]

    public async Task<IActionResult> Put([FromBody] EducationDTO body)
    {
        body.OrganisationChartId = currentUserDefaultOrganId;
        var result = await _EducationService.UpdateForAsync(body);
    ...
    public IActionResult GetPagedData(int currentPage = 0, int pageSize = 10, [FromQuery] string filter = "", [FromQuery] string activeSortColumn = "", [FromQuery] string Sortdirection = "", [FromQuery] bool IgnoreExpired = true, [FromQuery] long? EmployeeId = null)
    {
        ...
        return this.AppOk(_EducationService.GetPagedData(currentPage: currentPage, pageSize: pageSize, filter, activeSortColumn, Sortdirection, IgnoreExpired, EmployeeId: EmployeeId));
    ...
    {
        return this.AppOk(_EducationService.DeleteRecord(id));
    }
}
