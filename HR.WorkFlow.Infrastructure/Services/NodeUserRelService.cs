...
public class NodeUserRelService(IMapper mapper, IUnitOfWork<WorkFlowContext> unitOfWork, IDapper dapper, IConfiguration configuration, UserResolverService userService) : BaseService<HR.WorkFlow.Core.Data.NodeUserRel, WorkFlowContext, NodeUserRelDTO>(unitOfWork.Context, mapper, unitOfWork, dapper, configuration, userService), IScopedServices
{
    ...
    public new async Task<OperationResult> CreateForAsync(NodeUserRelDTO entityToCreate)
    {
    ...
    public new async Task<OperationResult> UpdateForAsync(NodeUserRelDTO entityToUpdate)
    {
